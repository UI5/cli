...
export default async function convertPackageLockToShrinkwrap(workspaceRootDir, targetPackageName) {
	...
	// Validate the generated shrinkwrap using Arborist
	await validateLockfile(shrinkwrap, targetNode);
...
/**
 * Validate the generated shrinkwrap structure using Arborist
 * This ensures the shrinkwrap is well-formed and can be loaded by npm/Arborist
 *
 * @param {object} shrinkwrap - Generated shrinkwrap object
 * @param {object} targetNode - Target package node from Arborist
 * @throws {Error} If the shrinkwrap is invalid or cannot be loaded by Arborist
 */
async function validateLockfile(shrinkwrap, targetNode) {
	let tempDir;
	try {
		// Create a temporary directory for validation
		tempDir = await mkdtemp(path.join(tmpdir(), "shrinkwrap-validation-"));

		// Create package.json (production dependencies only)
		const packageJson = {
			name: shrinkwrap.name,
			version: shrinkwrap.version,
			dependencies: targetNode.package.dependencies || {}
		};
		...
		// Attempt to load the virtual tree with Arborist
		// This validates that the shrinkwrap is well-formed
		const arb = new Arborist({
			path: tempDir,
		});
		...
		const errorMessage = error instanceof Error ? error.message : String(error);
		throw new Error(
			`Generated shrinkwrap validation failed: ${errorMessage}. ` +
			`The shrinkwrap structure is invalid or cannot be loaded by Arborist.`
		);
	} finally {
