...
import Component from "../../../../lib/specifications/types/Component.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const componentAPath = path.join(__dirname, "..", "..", "..", "fixtures", "component.a");
const componentHPath = path.join(__dirname, "..", "..", "..", "fixtures", "component.h");
...
test("Correct class", async (t) => {
	...
	t.true(project instanceof Component, `Is an instance of the Component class`);
});
...
test("getSourcePath", async (t) => {
	const {projectInput} = t.context;
	const project = await Specification.create(projectInput);
	t.is(project.getSourcePath(), path.join(componentAPath, "src"),
...
test("Access project resources w/ relative builder excludes", async (t) => {
	...
	projectInput.configuration.builder = {
		resources: {
			excludes: ["manifest.json"] // Has no effect since component excludes must be absolute or use wildcards
		}
	};
	const excludesProject = await Specification.create(projectInput);
	...
	t.is((await baselineProject.getReader({}).byGlob("**/manifest.json")).length, 1,
		"Found resource in baseline project for default style");
	t.is((await excludesProject.getReader({}).byGlob("**/manifest.json")).length, 1,
		"Did not find excluded resource for default style");

	t.is((await baselineProject.getReader({style: "buildtime"}).byGlob("**/manifest.json")).length, 1,
		"Found resource in baseline project for buildtime style");
	t.is((await excludesProject.getReader({style: "buildtime"}).byGlob("**/manifest.json")).length, 1,
		"Did not find excluded resource for buildtime style");

	t.is((await baselineProject.getReader({style: "dist"}).byGlob("**/manifest.json")).length, 1,
		"Found resource in baseline project for dist style");
	t.is((await excludesProject.getReader({style: "dist"}).byGlob("**/manifest.json")).length, 1,
		"Did not find excluded resource for dist style");

	t.is((await baselineProject.getReader({style: "flat"}).byGlob("**/manifest.json")).length, 1,
		"Found resource in baseline project for flat style");
	t.is((await excludesProject.getReader({style: "flat"}).byGlob("**/manifest.json")).length, 1,
		"Did not find excluded resource for flat style");
	...
	t.is((await baselineProject.getWorkspace().byGlob("**/manifest.json")).length, 1,
		"Found resource in baseline project for default style");
	t.is((await excludesProject.getWorkspace().byGlob("**/manifest.json")).length, 1,
		"Did not find excluded resource for default style");
});

test("Access project resources w/ incorrect builder excludes", async (t) => {
	const {projectInput} = t.context;
	const baselineProject = await Specification.create(projectInput);
...
test("Modify project resources via workspace and access via flat and runtime readers", async (t) => {
	...
	const newContent = (await workspaceResource.getString()).replace("Component A", "Some Name");
	...
	const runtimeReaderResource = await runtimeReader.byPath("/resources/id1/index.html");
	t.truthy(runtimeReaderResource, "Found the requested resource byPath");
	t.is(runtimeReaderResource.getPath(), "/resources/id1/index.html", "Resource (byPath) has correct path");
	...
	t.is(runtimeGlobResult[0].getPath(), "/resources/id1/index.html", "Resource (byGlob) has correct path");
...
test("_configureAndValidatePaths: Default paths", async (t) => {
	...
	t.is(project._srcPath, "src", "Correct default path");
});

test("_configureAndValidatePaths: Custom src directory", async (t) => {
	const componentHPath = path.join(__dirname, "..", "..", "..", "fixtures", "component.h");
	const projectInput = {
		id: "component.h.id",
		version: "1.0.0",
		...
		configuration: {
			...
			resources: {
				configuration: {
					paths: {
						src: "src-properties.componentName"
					}
				}
			}
		}
	...
	t.is(project._srcPath, "src-properties.componentName", "Correct path for src");
});

test("_configureAndValidatePaths: src directory does not exist", async (t) => {
	const {projectInput} = t.context;
	...
	t.is(err.message, "Unable to find source directory 'does/not/exist' in component project component.a");
});
...
test("_getNamespaceFromManifestJson: No 'sap.app' configuration found", async (t) => {
	...
	t.is(error.message, "No sap.app/id configuration found in manifest.json of project component.a",
		"Rejected with correct error message");
});

test("_getNamespaceFromManifestJson: No component id in 'sap.app' configuration found", async (t) => {
	const {projectInput, sinon} = t.context;
	const project = await Specification.create(projectInput);
...
test("_getNamespaceFromManifestAppDescVariant: No 'id' property found", async (t) => {
	...
	t.is(error.message, `No "id" property found in manifest.appdescr_variant of project component.a`,
		"Rejected with correct error message");
});
...
test("_getNamespace: Correct error message if fallback to manifest.appdescr_variant is not possible", async (t) => {
	...
	t.deepEqual(error.message,
		"Could not find required manifest.json for project component.a: " +
...
test("_getManifest: invalid JSON", async (t) => {
	...
	t.regex(error.message, /^Failed to read \/some-manifest\.json for project component\.a: /,
		"Rejected with correct error message");
...
test.serial("_getManifest: File does not exist", async (t) => {
	...
	t.deepEqual(error.message,
		"Failed to read /does-not-exist.json for project component.a: " +
		"Could not find resource /does-not-exist.json in project component.a",
		"Rejected with correct error message");
});
...
test.serial("_getManifest: Caches successes and failures", async (t) => {
	...
	const error = await t.throwsAsync(project._getManifest("/some-manifest.json"));
	t.deepEqual(error.message,
		"Failed to read /some-manifest.json for project component.a: " +
		"EPON: Pony Error",
		"Rejected with correct error message");
...
test("namespace: detect namespace from pom.xml via ${project.artifactId}", async (t) => {
	...
	componentHInput.configuration.resources.configuration.paths.src = "src-project.artifactId";
	const project = await Specification.create(componentHInput);

	t.is(project.getNamespace(), "component/h",
...
test("namespace: detect namespace from pom.xml via ${componentName} from properties", async (t) => {
	const {componentHInput} = t.context;
	componentHInput.configuration.resources.configuration.paths.src = "src-properties.componentName";
	const project = await Specification.create(componentHInput);
...
test("namespace: detect namespace from pom.xml via ${appId} from properties", async (t) => {
	const {componentHInput} = t.context;
	componentHInput.configuration.resources.configuration.paths.src = "src-properties.appId";

	const error = await t.throwsAsync(Specification.create(componentHInput));
	t.deepEqual(error.message, "Failed to resolve namespace of project component.h: \"${appId}\"" +
