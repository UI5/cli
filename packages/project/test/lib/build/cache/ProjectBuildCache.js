...
test("discardIncrementalState clears the retained result signature and resets ProjectResources", async (t) => {
	...
	t.true(resetStub.callCount > resetCallsBefore,
		"discardIncrementalState resets the project's stage pipeline so cached stages re-import");
...
test("discardIncrementalState is a no-op in Cache.Off mode", async (t) => {
	const project = createMockProject();
	const cacheManager = createMockCacheManager();
	const cache = new ProjectBuildCache(project, "sig", cacheManager, Cache.Off);
	await cache.initSourceIndex();
...
async function createCacheInRestoringState({
	...
	const cache = new ProjectBuildCache(project, buildSignature, cacheManager, cacheMode);
	await cache.initSourceIndex();
...
async function createCacheWithDependencyGlob({
	...
	const cache = new ProjectBuildCache(project, "build-signature", cacheManager);
	await cache.initSourceIndex();
