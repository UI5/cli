...
// throw the unmappable-path error. The event is dropped (no `change`, no fatal `error`), so a branch
// switch does not escalate the server to terminal ERROR.
...
test.serial("watch: skips subscribe on a missing source path", async (t) => {
	...
	existsStub.withArgs("/gone").resolves(false);
	existsStub.withArgs("/src").resolves(true);
...
// source dir. It must propagate so BuildServer's loop-protected recovery applies.
test.serial("watch: rejects when subscribe fails on an existing path", async (t) => {
	...
	existsStub.withArgs("/src").resolves(true); // path present -> genuine fault
