...
export default class ProjectBuildCache {
	...
	async prepareTaskExecution(taskName, requiresDependencies) {
		...
		if (taskCache) {
			...
			if (stageCache) {
				...
				const deltaStageCache = await this.#findStageCache(stageName, [deltaInfo.signature]);
				if (deltaStageCache) {
					log.info(`Using delta cached stage for task ${taskName} in project ${this.#project.getName()}`);
					return {
	...
	async recordTaskResult(taskName, projectResourceRequests, dependencyResourceRequests, cacheInfo) {
		...
		if (cacheInfo) {
			...
			let reader;
			if (!cacheInfo.previousStageCache.byGlob) {
				// Stage instance
				reader = cacheInfo.previousStageCache.getWriter() ?? cacheInfo.previousStageCache.getReader();
			}
