 ...
 * middleware, for integration into an existing HTTP server rather than starting one.
 *
 * Unlike {@link module:@ui5/server.serve}, this does not bind a port, attach the live-reload
 * WebSocket server, or install the terminal HTML error handler — those are concerns of the
 * server the UI5 CLI owns. The caller mounts the returned <code>middleware</code> on its own
 * <code>express()</code> app or Express <code>Router</code>/Connect app via
 * <code>app.use(middleware)</code> and remains responsible for error handling and the HTTP
 * listener.
 *
 * <code>close</code> must be called on teardown to release the underlying BuildServer's source
 * watcher and build-cache handle.
 *
...
export default async function serveMiddleware(graph, {
	...
	const config = {
		...
		ui5DataDir, includedTasks, excludedTasks,
		// The live-reload WebSocket server is owned by the UI5 CLI's HTTP server, which this
		// embedding API does not create. Keeping it inactive with no token leaves the
		// liveReloadClient middleware in place but dormant (no client script injection).
		liveReload: false,
		webSocketToken: null,
	};
