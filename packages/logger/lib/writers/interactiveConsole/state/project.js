...
export function createProjectState() {
	return {
		...
		// When true, the version slot(s) render a dim "resolving" placeholder while the project
		// name and type keep showing. Enabled by `ui5.project-resolve-started` once a definition
		// change is known to be coming (a `git checkout`, a ui5.yaml edit) and the resolved version
		// is about to change. `setProject` clears it; a failed re-resolve releases it via
...
// with no `ui5.project-resolve-succeeded`.
